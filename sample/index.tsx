  ...
  useEffect(() => {
    ...
    const unsubscribeGames = onSnapshot(gamesQuery, (snapshot) => {
      ...
      if (filterIds && filterIds.length > 0) {
        publishedGames = publishedGames.filter((g) => filterIds.includes(g.id));
      }
      if (favoritesOnly) {
      ...
      setGames(
        publishedGames.map((game) => ({
          ...game,
          isFavorite: userPrefs.favorites?.includes(game.id) || false,
          playCount: userPrefs.playCounts?.[game.id] || 0,
          recommendationScore:
            (userPrefs.favorites?.includes(game.id) ? 10 : 0) +
            (userPrefs.playCounts?.[game.id] || 0),
        }))
      );
      ...
      publishedGames.forEach((g) => {
        updatedCounts[g.id] = (g as any).likeCount || 0;
      });
      setLikeCounts(updatedCounts);
    });

    return () => unsubscribeGames();
  }, [filterIds, favoritesOnly, userPrefs]);
